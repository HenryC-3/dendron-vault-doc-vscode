...
![Emmet in suggestion/auto-completion list](/assets/emmet-gwh14zqfq19m.gif)
...
If you have disabled the `editor.quickSuggestions` [[vs.1.7.docs.getstarted.settings]], you won't see suggestions as you type. You can still trigger suggestions manually by pressing `kb(editor.action.triggerSuggest)` and see the preview.
...
To enable the Emmet abbreviation expansion in file types where it is not available by default, use the `emmet.includeLanguages` setting. Make sure to use [[vs.1.7.docs.languages.identifiers]] for both sides of the mapping, with the right side being the language identifier of an Emmet supported language (see the list above).
...
![Emmet with multi cursors](/assets/emmet-multi-cursor-ei0e7utv98nw.gif)
...
If you use the [[vs.1.7.http:..getbem.com.]] (BEM) way of writing HTML, then `bem` filters are very handy for you to use. To learn more about how to use `bem` filters, read [BEM filter in Emmet](https://docs.emmet.io/filters/bem/).
...
Below are Emmet [[vs.1.7.docs.getstarted.settings]] that you can use to customize your Emmet experience in VS Code.
...
* [[vs.1.7.docs.languages.html]] - VS Code supports HTML with IntelliSense, closing tags, and formatting.
* [[vs.1.7.docs.languages.css]] - We offer rich support for CSS, SCSS and Less.
...
* Check the [[vs.1.7.docs.editor.extension-marketplace.md#extensions-view-filters]] to see if Emmet has been disabled.
* Try restarting the extension host by running the **Developer: Restart Extension Host** (`workbench.action.restartExtensionHost`) command in the [[vs.1.7.docs.getstarted.userinterface.md#command-palette]].

### Where can I set all the preferences as documented in [Emmet preferences](https://docs.emmet.io/customization/preferences/)?

You can set the preferences using the setting `emmet.preferences`. Only a subset of the preferences that are documented in [Emmet preferences](https://docs.emmet.io/customization/preferences/) can be customized. Please read the preferences section under [[vs.1.7.docs.editor.emmet.md#emmet-configuration]].
...
* Don't hesitate to create your own [[vs.1.7.docs.editor.emmet.md#using-custom-emmet-snippets]].
