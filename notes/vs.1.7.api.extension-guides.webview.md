...
- As the view for a [[vs.1.7.api.extension-guides.custom-editors]]. Custom editors allow extensions to provide a custom UI for editing any file in the workspace. The custom editor API also lets your extension hook into editor events such as undo and redo, as well as file events such as save.
- In [[vs.1.7.api.references.vscode-api#WebviewView]] that are rendered in the sidebar or panel areas. See the [webview view sample extension](https://github.com/microsoft/vscode-extension-samples/tree/main/webview-view-sample) for more details.
...
- [[vs.1.7.api.references.vscode-api#window.createWebviewPanel]]
- [[vs.1.7.api.references.vscode-api#window.registerWebviewPanelSerializer]]
...
Here's the `package.json` for the first version of the **Cat Coding** extension. You can find the complete code for the example app [here](https://github.com/microsoft/vscode-extension-samples/blob/main/webview-sample/README.md). The first version of our extension [[vs.1.7.api.references.contribution-points#contributes.commands]] called `catCoding.start`. When a user invokes this command, we will show a simple webview with our cat in it. Users will be able to invoke this command from the **Command Palette** as **Cat Coding: Start new cat coding session** or even create a keybinding for it if they are so inclined.
...
![An empty webview](/assets/basics-no_content-1dfm25r73lpd.png)
...
![A webview with some HTML](/assets/basics-html-xxs0hvthci22.png)
...
![Updating the webview content](/assets/basics-update-kp3h2hbsgqgl.gif)
...
![Webview content is automatically restored when the webview becomes visible again](/assets/basics-restore-fc4mry91k149.gif)
...
![Webviews are moved when you drag them between tabs](/assets/basics-drag-xc0xnmtosmj3.gif)
...
![Using a single panel and reveal](/assets/basics-single_panel-qscrie22d41a.gif)
...
![Responding to onDidChangeViewState events](/assets/basics-ondidchangeviewstate-f2bk24znyu4k.gif)
...
![The developer tools](/assets/developer-overview-j9patcz3x2ur.png)
...
![Inspecting a webview using the developer tools](/assets/developer-inspect-25j8bdx75om5.png)

You can also view all of the errors and logs from your webview in the developer tools console:

![The developer tools console](/assets/developer-console-7vepvfxzqwme.png)
...
![Selecting the active frame](/assets/developer-active-frame-79udal2hz8vj.png)
...
Review the [[vs.1.7.api.references.theme-color]] for the available theme variables. [An extension](https://marketplace.visualstudio.com/items?itemName=connor4312.css-theme-completions) is available which provides IntelliSense suggestions for the variables.
...
![A script running in a webview](/assets/scripts-basic-phqopxc5ut3a.gif)
...
![Passing messages to a webview](/assets/scripts-extension_to_webview-bel93fr4ux90.gif)
...
![Passing messages from the webview to the main extension](/assets/scripts-webview_to_extension-gszexoisnhaw.gif)
...
![persistence retrain](/assets/persistence-retrain-udzvpudrhd7w.gif)
...
- [[vs.1.7.api]] - Learn about the full VS Code Extension API.
- [[vs.1.7.api.extension-capabilities.overview]] - Take a look at other ways to extend VS Code.
