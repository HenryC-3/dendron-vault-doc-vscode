...
> **Note**: Your Linux VM can be hosted anywhere - on your local host, on premise, in Azure, or in any other cloud, as long as the chosen Linux distribution meets these [[vs.1.7.docs.remote.linux.md#local-linux-prerequisites]].

## Prerequisites

To get started, you need to have done the following steps:

1. Install an [[vs.1.7.docs.remote.troubleshooting.md#installing-a-supported-ssh-client]] (PuTTY is not supported).
...
![Remote - SSH extension](/assets/remote-ssh-extension-9ofqmzau2hqh.png)

### Remote - SSH

With the Remote - SSH extension installed, you will see a new Status bar item at the far left.

![Remote Status bar item](/assets/remote-status-bar-krtbf9449bja.png)

The Remote Status bar item can quickly show you in which context VS Code is running (local or remote) and clicking on the item will bring up the Remote - SSH commands.

![Remote - SSH commands](/assets/remote-ssh-commands-32xgirvohmak.png)
...
> **Note**: In this tutorial, we are using Azure, but your Linux VM can be hosted anywhere, as long as the Linux distribution meets these [[vs.1.7.docs.remote.linux.md#local-linux-prerequisites]].

![Create a virtual machine](/assets/create-vm-r268zkfez9ag.png)

Now you can specify details of your VM, such as the name, the size, and the base image. Choose Ubuntu Server 18.04 LTS for this example, but you can choose recent versions of other Linux distros and look at VS Code's [[vs.1.7.docs.remote.troubleshooting.md#installing-a-supported-ssh-server]].

![Virtual machine instance details](/assets/vm-instance-details-rix7xobim3x6.png)

## Set up SSH

There are several authentication methods into a VM, including an SSH public/private key pair or a username and password. We recommend using key-based authentication (if you use a username/password, you'll be prompted to enter your credentials more than once by the extension). If you're on Windows and have already created keys using PuttyGen, you can [[vs.1.7.docs.remote.troubleshooting.md#reusing-a-key-generated-in-puttygen]].
...
![ssh-keygen output](/assets/ssh-keygen-mfx7jr89cq95.png)
...
![Add SSH public key to VM](/assets/add-ssh-public-key-wfpb4wp5yjxx.png)
...
![Virtual machine auto-shutdown](/assets/vm-auto-shutdown-t8xgfhsb7g5o.png)
...
![Remote extension commands](/assets/remote-commands-1gfacjvgf96w.png)
...
![Virtual machine public IP address](/assets/vm-public-ip-address-9nl2zjga7907.png)
...
> Note: If you run into an error `ssh: connect to host <host ip> port 22: Connection timed out`, you may need to delete NRMS-Rule-106 from the Networking tab of your VM:
   ...
   ![Virtual machine list of NRMS rules](/assets/vm-nrms-rules-mhbmnfr21dwa.png)

Set the user and hostname in the connection information text box.

![Set user and host name](/assets/set-user-host-5t054bne5vi4.png)
...
![Initializing VS Code Server](/assets/init-vs-code-server-9eefdra9vs70.png)

You'll know you're connected to your VM by looking at the indicator in the Status bar. It shows the hostname of your VM.

![SSH indicator in Status bar](/assets/ssh-status-bar-0tomjeb6nteg.png)
...
![Remote button on Activity bar](/assets/remote-on-activity-bar-qv488ngi1jhe.png)
...
![Checking uname in the terminal](/assets/check-uname-fdq73ppr0h4i.png)
...
![Remote open folder](/assets/remote-open-folder-haa3712gerce.png)
...
The Express app by default runs on [[vs.1.7.http:..localhost:3000]]. You won't see anything in your local browser on localhost:3000 because the web app is running on your virtual machine.

### Port forwarding

To be able to browse to the web app on your local machine, you can leverage another feature called [[vs.1.7.docs.remote.ssh.md#temporarily-forwarding-a-port]].
...
![Enter the port to forward](/assets/enter-port-3k8e7if4uqpc.png)

Name the connection "browser":

![Name the port](/assets/name-port-5kbxxeknfvty.png)

The server will now forward traffic on port 3000 to your local machine. When you browse to [[vs.1.7.http:..localhost:3000]], you see the running web app.

![Running Express Application](/assets/express-zrmx9kqsuxog.png)
...
![Express app.js hover](/assets/express-hover-ij9hh7tdvr3z.png)

When you start typing, you'll get smart completions for the object methods and properties.

![Express app.js smart completions](/assets/express-completions-ma2tbqve0tqu.png)
...
![set breakpoint](/assets/set-breakpoint-lwcfxbcrdnir.png)
...
![VS Code debug view](/assets/debug-view-p1zfwv2jlxry.png)
