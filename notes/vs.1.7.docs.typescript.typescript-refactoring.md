...
![Renaming a method](/assets/rename-n7xunid0msqy.png)
...
![TypeScript refactoring](/assets/refactorings-exo00uqhuo81.png)
...
* **Extract to method or function** - Extract the selected statements or expressions to either a new method or a new function in the file.
   ...
   ![Triggering the extract method refactoring on a selection](/assets/refactor-extract-function-vep5q9qqpnyw.gif)
...
* **Extract to constant** - Extract the selected expression to a new constant in the file.
   ...
   ![Extracting a constant from a selection](/assets/refactor-extract-constant-fsxtz6wg60hy.gif)

* **Extract type to interface or type alias** - Extract the selected complex type to either an interface or a type alias.

   ![Extract an inline type to an interface](/assets/refactor-extract-interface-ajravn244s71.gif)
...
* **Move to new file** - Move one or more classes, functions, constants, or interfaces in the top-level scope of the file to a new file. The new file's name is inferred from the selected symbol's name.
   ...
   ![Moving a class to a new file](/assets/refactor-move-file-iwuxjbs5jq9k.gif)
...
* **Convert between named imports and namespace imports** - Convert between named imports (`import { Name } from './foo'`) and namespace imports (`import * as foo from './foo'`).
   ...
   ![Converting a named import to a namespace import](/assets/refactor-convert-import-sju8kjccyssc.gif)
...
* **Generate get and set accessors** - Encapsulate a selected class property by generating a getter and setter for it.
   ...
   ![Generating getters and setters from class property](/assets/refactor-generate-get-set-tsydogt0abej.gif)

* **Infer function return types** - Adds explicit return type annotations to functions.

    ![The Infer function return type refactoring in action](/assets/ts-infer-return-oxzqhc55a6sy.gif)
...
![Unreachable source code faded out](/assets/unreachable-rh083j7rp963.png)
...
See [[vs.1.7.docs.typescript.typescript-tutorial]] for more information.
...
* [[vs.1.7.docs.typescript.typescript-editing]] - Learn about VS Code editing features for TypeScript.
