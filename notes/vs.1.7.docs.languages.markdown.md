---
id: 3ag140zw3d3yes8zec9ieui
title: Markdown
desc: ""
updated: 1660824526171
...
![Markdown Preview](/assets/preview-079lqvwcd10g.png)
...
![The preview automatically switching to preview the current Markdown document](/assets/md-dynamic-preview-qk620tjuq73e.gif)
...
![A locked Markdown preview](/assets/locked-preview-title-cnyhnojyl020.png)
...
![Markdown Preview editor selection scroll sync](/assets/preview-scroll-sync-k5b5xdoni638.gif)

You can disable scroll synchronization using the `markdown.preview.scrollPreviewWithEditor` and `markdown.preview.scrollEditorWithPreview` [[vs.1.7.docs.getstarted.settings]].

The currently selected line in the editor is indicated in the Markdown preview by a light gray bar in the left margin:

![Markdown Preview editor line marker](/assets/preview-selection-marker-tbn1xcg44xbk.png)
...
![Markdown Preview double click switches to editor](/assets/double-click-preview-switch-3myvpgm7mjau.gif)
...
![Markdown Outline view](/assets/markdown-outline-view-bhibyj7s3fan.png)
...
You can also use your own CSS in the Markdown preview with the `"markdown.styles": [[vs.1.7.docs.getstarted.settings.md]]. This lists URLs for style sheets to load in the Markdown preview. These stylesheets can either be `https` URLs, or relative paths to local files in the current workspace.
...
![Markdown security alert](/assets/security-alert-6piepf7q01h9.png)
...
![Markdown security selector](/assets/security-selector-360eefjo5uf8.png)
...
> **Tip:** You can add in your own User Defined Snippets for Markdown. Take a look at [[vs.1.7.docs.editor.userdefinedsnippets]] to find out how.

## Compiling Markdown into HTML

VS Code integrates with Markdown compilers through the integrated [[vs.1.7.docs.editor.tasks]]. We can use this to compile `.md` files into `.html` files. Let's walk through compiling a simple Markdown document.
...
To complete the tasks integration with VS Code, we will need to modify the task configuration from before to run the default Gulp task we just created. You can either delete the `tasks.json` file or empty it, only keeping the `"version": "2.0.0"` property. Now execute **Run Task** from the global **Terminal** menu. Observe that you are presented with a picker listing the tasks defined in the gulp file. Select **gulp: default** to start the task. We allow you to scan the output for compile problems. Since we only want to convert the Markdown file to HTML, select **Never scan the build output** from the presented list. At this point, if you create and/or modify other Markdown files, you see the respective HTML files generated and/or changes reflected on save. You can also enable [[vs.1.7.docs.editor.codebasics.md#save-auto-save]] to make things even more streamlined.
...
-   [[vs.1.7.docs.languages.css]] - Want to edit your CSS? VS Code has great support for CSS, SCSS, and Less editing.
