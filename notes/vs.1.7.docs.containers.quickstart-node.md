...
MetaDescription: >-
  Develop, build, and debug a Node.js app in a Docker container, using Visual
  Studio Code.
---
...
- Both Docker and the VS Code Docker extension must be installed as described in the [[vs.1.7.docs.containers.overview.md#installation]]
...
1. Open the Command Palette (`kb(workbench.action.showCommands)`) and use **Docker: Add Docker Files to Workspace...** command:
   ...
   ![Add Dockerfile to a Node.js project](/assets/node-add-node-dark-rn0j2munie12.png)
...
The Docker extension helps you author Dockerfiles by using [[vs.1.7.docs.editor.intellisense]] to provide auto-completions and contextual help. To see this feature in action, add an environment variable to your service image by following these steps:

1. Open the `Dockerfile` file.
1. Use `ENV` instruction to add an environment variable to the service container image.

   ![Add an environment variable to Docker image](/assets/nodejs-intellisense-env-8822l3wqf7l3.png)
...
1. Open the web browser and navigate to [[vs.1.7.http:..localhost:3000]]. You should see a page similar to the following:

   ![Application page in browser](/assets/node-run-browser-o39cntbwddsf.png)
...
1. Open the Docker Explorer and verify that the new image is visible in the Images tree:
   ...
   ![Verify Docker image exists](/assets/node-verify-image-dark-8dzi4dxwr3hl.png)
...
1. Right-click on the image built in the previous section and select **Run** or **Run Interactive**. The container should start and you should be able to see it in the Docker Containers tree:
   ...
   ![Running service container](/assets/node-running-container-dark-n25vv1a8o0j4.png)
...
1. Make sure the **Docker Node.js Launch** debugger configuration is selected.
   ...
   ![Selected Docker debug configuration](/assets/node-debug-configuration-dark-h72dwul6215a.png)
...
1. Start debugging (use the `kb(workbench.action.debug.start)` key).
    ...
    >
    > You can configure the application to wait for the debugger to attach before starting execution by setting the [[vs.1.7.docs.containers.reference.md#node-object-properties-dockerrun-task]] property to `break` in the `docker-run: debug` task in `tasks.json` under the `node` object.
...
1. In the **Containers** tab, right-click on your container and choose **View Logs**.
    ...
    ![Screenshot of logs in the terminal](/assets/node-view-logs-dark-2gzmlei7tldw.png)
