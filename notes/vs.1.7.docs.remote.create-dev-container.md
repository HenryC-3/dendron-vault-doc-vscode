---
id: mwqbsmrg8e3g5k81o5zwtc3
title: Create Dev Container
desc: ''
...
* Add more [[vs.1.7.remote.advancedcontainers.overview]].
...
![Quick pick with list of Remote-Containers commands](/assets/remote-containers-reopen-3upkxlgty52e.png)
...
![VS Code instance connected to dev container](/assets/connected-to-dev-container-sz4p79m1qctu.png)
...
> **Tip:** Want to use a remote Docker host? See the [[vs.1.7.remote.advancedcontainers.develop-remote-host]] article for details on setup.
...
See the article on [[vs.1.7.docs.remote.containers.md#dev-container-features-preview]] for more details.
...
![Add a dev container definition](/assets/select-dev-container-def-all-3wts32u2lgzg.png)
...
![Select Dockerfile](/assets/select-dockerfile-akucatfo2vb5.png)
...
![Container edit loop illustration](/assets/container-edit-loop-gb54pzv3vo9q.png)
...
![Select Docker Compose File](/assets/select-docker-compose-vkdjb312v0eo.png)
...
See the [[vs.1.7.docs.remote.devcontainerjson-reference]] for information other available properties such as the `workspaceFolder` and `shutdownAction`.
...
However, on Linux you may need to set up and **specify a non-root user** when using a bind mount or any files you create will be root. See [[vs.1.7.remote.advancedcontainers.add-nonroot-user]] for details. To have VS Code run as a different user, add this to `devcontainer.json`:
...
![Dev container configuration file reopen notification](/assets/dev-container-reopen-prompt-74w6xct2d7ql.png)
...
You may also add a badge or link in your repository so that users can easily open your project in Remote - Containers. It will install the Remote - Containers extension if necessary, clone the repo into a container [[vs.1.7.remote.advancedcontainers.improve-performance.md#use-clone-repository-in-container-volume]], and start up the dev container.
...
First, update the **Remote > Containers: Repository Configuration Paths** [[vs.1.7.docs.getstarted.settings]] with the local folder you want to use to store your repository container configuration files.

In the Settings editor:

![Repository container folders setting](/assets/repo-container-folder-setting-3j7ra4p078h8.png)
