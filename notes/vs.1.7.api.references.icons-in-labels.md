...
The product icons that ship with VS Code are contained in the [Codicon icon font](https://github.com/microsoft/vscode-codicons) and form the **default** product icon theme. Extensions can provide new [[vs.1.7.api.extension-guides.product-icon-theme]] to redefine these icons and give VS Code a new appearance.
...
Icons can be used in Markdown labels in hovers, in the  [[vs.1.7.api.references.vscode-api#StatusBarItem]] text and [[vs.1.7.api.references.vscode-api#QuickPickItem]] label API. The syntax for adding an icon in Markdown is `$(iconIdentifier)`:
...
[[vs.1.7.api.extension-guides.product-icon-theme]] can replace each icon individually, as well as all icons from the codicon library.
