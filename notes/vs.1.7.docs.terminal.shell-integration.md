...
MetaDescription: >-
  Visual Studio Code's embedded terminal can integrate with some shells to
  enhance the capabilities of the terminal.
---
...
This standard, easy way will not work for some advanced use cases like in sub-shells, through a regular `ssh` session (when not using the [[vs.1.7.docs.remote.ssh]]) or for some complex shell setups. The recommended way to enable shell integration for those is [[v[[vs.1.7.#manual-installation]]
...
![Blue circles appear next to successful commands, red circles with crosses appear next to failed commands. The color of the circles appears in the scroll bar](/assets/decorations-vx8dv96ovmp0.png)

The decorations can be interacted with to give some contextual actions like re-running the command:

![Clicking a successful command decoration shows a context menu containing items: Copy Output, Copy Output as HTML, Rerun Command and How does this work?](/assets/decoration-menu-r2zcfobeo9ji.png)
...
![The "run recent command" command shows a quick pick with previously run commands that can be filtered similar to the go to file command](/assets/recent-command-4wwttejw5ytk.png)
...
![When the sequence is written to the terminal a small grey circle will appear to the left of the command, with a matching annotation in the scroll bar](/assets/setmark-1ngeea65y8fp.png)
