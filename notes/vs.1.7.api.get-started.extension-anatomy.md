...
- Registers the [[vs.1.7.api.references.activation-events#onCommand]] [[vs.1.7.api.references.activation-events]]: `onCommand:helloworld.helloWorld`, so the extension becomes activated when user runs the `Hello World` command.
- Uses the [[vs.1.7.api.references.contribution-points#contributes.commands]] [[vs.1.7.api.references.contribution-points]] to make the command `Hello World` available in the Command Palette, and bind it to a command ID `helloworld.helloWorld`.
- Uses the [[vs.1.7.api.references.vscode-api#commands.registerCommand]] [[vs.1.7.api.references.vscode-api]] to bind a function to the registered command ID `helloworld.helloWorld`.
...
- `launch.json` used to configure VS Code [[vs.1.7.docs.editor.debugging]]
- `tasks.json` for defining VS Code [[vs.1.7.docs.editor.tasks]]
...
- `activationEvents` and `contributes`: [[vs.1.7.api.references.activation-events]] and [[vs.1.7.api.references.contribution-points]].
