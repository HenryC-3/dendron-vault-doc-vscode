...
1. To use a development container locally, [[vs.1.7.docs.remote.containers#_getting-started]], and use **File > Open... / Open Folder...** to open your source code locally in VS Code. To use Codespaces instead, navigate to the repository that contains your extension on GitHub and [open it in a codespace](https://docs.github.com/github/developing-online-with-codespaces/creating-a-codespace) to work with it in a browser-based editor. You can also [open the codespace in VS Code](https://docs.github.com/en/github/developing-online-with-codespaces/using-codespaces-in-visual-studio-code) if you prefer.
...
3. **Optional:** After this command runs, you can modify the contents of the `.devcontainer` folder to include additional build or runtime requirements. See the in-depth [[vs.1.7.docs.remote.create-dev-container#_set-up-a-folder-to-run-in-a-container]] documentation for details.
...
1. After [[vs.1.7.docs.remote.ssh#_getting-started]], select **Remote-SSH: Connect to Host...** from the Command Palette (`kbstyle(F1)`) in VS Code to connect to a host.
...
1. After [[vs.1.7.docs.remote.wsl]], select **Remote-WSL: New Window** from the Command Palette (`kbstyle(F1)`) in VS Code.
...
3. Connect to a [codespace](https://docs.github.com/github/developing-online-with-codespaces), [[vs.1.7.docs.remote.containers]], [[vs.1.7.docs.remote.ssh]], or [[vs.1.7.docs.remote.wsl]].
...
If your extension needs to preserve some user state across different machines then provide the state to [[vs.1.7.docs.editor.settings-sync]] using `vscode.ExtensionContext.globalState.setKeysForSync`. This can help prevent displaying the same welcome or updates page to users on multiple machines.

There is an example of using `setKeysforSync` in the [[vs.1.7.api.extension-capabilities.common-capabilities#data-storage]] topic.
...
See the [[vs.1.7.api.extension-guides.command]] for details on working with commands.

## Using the Webview API

Like the clipboard API, the [[vs.1.7.api.extension-guides.webview]] is always run on the user's local machine or in the browser, even when used from a Workspace Extension. This means that many webview-based extensions should just work, even when used in remote workspaces or Codespaces. However, there are some considerations to be aware of so that your webview extension works properly when run remotely.

### Always use asWebviewUri

You should use the `asWebviewUri` API to manage extension resources. Using this API instead of hard coding `vscode-resource://` URIs is required to ensure the Codespaces browser-based editor works with your extension. See the [[vs.1.7.api.extension-guides.webview]] guide for details, but here is a quick example.
...
![Webview Solution](/assets/webview-solution-07ol8ab92cd7.png)
...
- See [[vs.1.7.docs.remote.troubleshooting]] or the [[vs.1.7.docs.remote.faq]].
