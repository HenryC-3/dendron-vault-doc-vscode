...
Visual Studio Code supports variable substitution in [[vs.1.7.docs.editor.debugging]] and [[v[[vs.1.7.docs.editor.tasks]]nfiguration files as well as some select settings. Variable substitution is supported inside some key and value strings in `launch.json` and `tasks.json` files using **${variableName}** syntax.
...
The predefined variables are supported in a select number of setting keys in `settings.json` files such as the terminal `cwd`, `env`, `shell` and `shellArgs` values. Some [[vs.1.7.docs.getstarted.settings]] like `window.title` have their own variables:
...
The variable `${workspaceRoot}` was deprecated in favor of `${workspaceFolder}` to better align with [[vs.1.7.docs.editor.multi-root-workspaces]] support.
...
One easy way to check a variable's runtime value is to create a VS Code [[vs.1.7.docs.editor.tasks]] to output the variable value to the console. For example, to see the resolved value for `${workspaceFolder}`, you can create and run (**Terminal** > **Run Task**) the following simple 'echo' task in `tasks.json`:
