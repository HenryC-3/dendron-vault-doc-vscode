---
id: av9njb2ouls2o97zg7a7d5v
title: Debugging
desc: ''
updated: 1660824078457
created: 1660824078457
isDir: false
Order: 4
Area: python
TOCTitle: Debugging
...
MetaDescription: >-
  Details on configuring the Visual Studio Code debugger for different Python
  applications.
MetaSocialImage: images/tutorial/social.png
---
# Python debugging in VS Code

The Python extension supports debugging of several types of Python applications. For a short walkthrough of basic debugging, see [[vs.1.7.docs.python.python-tutorial.md#configure-and-run-the-debugger]]. Also see the [[vs.1.7.docs.python.tutorial-flask]]. Both tutorials demonstrate core skills like setting breakpoints and stepping through code.

For general debugging features such as inspecting variables, setting breakpoints, and other activities that aren't language-dependent, review [[vs.1.7.docs.editor.debugging]].
...
![Run icon](/assets/debug-icon-7758d9itidoy.png)
...
![Debug toolbar settings command](/assets/debug-start-8kn2hrjud9h8.png)
...
![Debug configurations menu](/assets/debug-configurations-p65fqp3ze2l4.png)
...
![Adding a new Python debugging configuration](/assets/add-configuration-koe6o3qxr1gl.png)
...
![Debugging Status Bar](/assets/debug-status-bar-md6e7dgb3ct1.png)
...
![Debug button on the top-right of the editor](/assets/debug-button-editor-9pawc06vtczl.png)
...
![Show all automatic debug configurations option on the run view](/assets/debug-auto-config-1g7l8atldj1d.png)
...
![Run the debugger](/assets/debug-run-hmgi517fjp3u.png)
...
1. During remote debugging, the debugging toolbar appears as below:
    ...
    ![Debugging toolbar during remote debugging](/assets/remote-debug-toolbar-tk8gvl3a3vb5.png)
...
| `"integratedTerminal"` (default) | [[vs.1.7.docs.terminal.basics]]. If `redirectOutput` is set to True, output is also displayed in the debug console.|
...
Specifies whether to enable subprocess debugging. Defaults to `false`, set to `true` to enable. For more information, see [[vs.1.7.docs.editor.debugging.md#multitarget-debugging]].
...
Optional path to a file that contains environment variable definitions. See [[vs.1.7.docs.python.environments.md#environment-variable-definitions-file]].
...
The Python extension supports [[vs.1.7.docs.editor.debugging.md#breakpoints]] and [[vs.1.7.docs.editor.debugging.md#logpoints]] for debugging code. For a short walkthrough of basic debugging and using breakpoints, see [[vs.1.7.docs.python.python-tutorial.md#configure-and-run-the-debugger]].

### Conditional breakpoints

Breakpoints can also be set to trigger based on expressions, hit counts, or a combination of both. The Python extension supports hit counts that are integers, in addition to integers preceded by the ==, >, >=, <, <=, and % operators. For example, you could set a breakpoint to trigger after five occurrences by setting a hit count of `>5`  For more information, see [[vs.1.7.docs.editor.debugging.md#conditional-breakpoints]] in the main VS Code debugging article.
...
Specific steps are also needed for remote debugging and Google App Engine. For details on debugging tests, see [[vs.1.7.docs.python.testing]].
...
- The path to the python executable is incorrect: check the path of your selected interpreter by running the **Python: Select Interpreter** command and looking at the current value:
  ...
  ![Troubleshooting wrong Python interpreter when debugging](/assets/debug-troubleshooting-wrong-path-y4tl3pc29cd4.png)
...
- [[vs.1.7.docs.editor.debugging]] - Learn about the debugging features of VS Code.
