---
id: 9unqidbh0nbybtc3xat2maj
title: TypeScript Compiling
desc: ''
updated: 1660824078463
created: 1660824078463
isDir: false
Order: 2
Area: typescript
TOCTitle: Compiling
...
![tsconfig.json IntelliSense](/assets/tsconfigintellisense-vazw9u1l9guo.png)

A simple `tsconfig.json` looks like this for ES5, **CommonJS** [[vs.1.7.http:..www.commonjs.org.specs.modules.1.0]] and source maps:
...
VS Code integrates with `tsc` through our integrated [[vs.1.7.docs.editor.tasks]]. We can use this to transpile `.ts` files into `.js` files. Another benefit of using VS Code tasks is that you get integrated error and warning detection displayed in the [[v[[vs.1.7.docs.editor.editingevolved.md#errors-warnings]]nel. Let's walk through transpiling a simple TypeScript Hello World program.
...
![build and run Hello World](/assets/build-hello-world-g3i9xetewt1a.png)
...
![TypeScript Build](/assets/typescript-build-71gwmeooyq07.png)
...
> **Tip:** You can also run the program using VS Code's Run/Debug feature. Details about running and debugging Node.js applications in VS Code can be found in the [[vs.1.7.docs.nodejs.nodejs-tutorial.md#debug-your-express-app]]

### Step 4: Reviewing build issues

The VS Code task system can also detect build issues through a [[vs.1.7.docs.editor.tasks.md#defining-a-problem-matcher]]. A problem matcher parses build output based on the specific build tool and provides integrated issue display and navigation. VS Code ships with many problem matchers and `$tsc` seen above in `tasks.json` is the problem matcher for TypeScript compiler output.
...
![Error in Status Bar](/assets/error-status-bar-5yw9tli8iyxj.png)

You can also use the keyboard to open the list `kb(workbench.actions.view.problems)`.

>**Tip:** Tasks offer rich support for many actions. Check the [[vs.1.7.docs.editor.tasks]] topic for more information on how to configure them.
...
When you are working with TypeScript, you often don't want to see generated JavaScript files in the File Explorer or in Search results. VS Code offers filtering capabilities with a `files.exclude` [[vs.1.7.docs.getstarted.settings]] and you can easily create an expression to hide those derived files:
...
![Hiding derived resources](/assets/hidingderivedbefore-jgjg9crcpvfq.p![Hiding derived resources](/assets/hidingderivedafter-wk2swsdihr31.png)png)
...
![TypeScript status bar version](/assets/status-bar-version-iexremzm7je2.png)
...
![TypeScript version selector](/assets/select-ts-version-message-2ab9johf6nwd.png)
...
VS Code will automatically detect workspace versions of TypeScript that are installed under `node_modules` in the root of your workspace. You can also explicitly tell VS Code which version of TypeScript to use by configuring the `typescript.tsdk` in your user or workspace [[vs.1.7.docs.getstarted.settings]]. The `typescript.tsdk` setting should point to a directory containing the TypeScript `tsserver.js` file. You can find the TypeScript installation location using `npm list -g typescript`. The `tsserver.js` file is usually in the `lib` folder.
...
You can disable this behavior by setting `"typescript.reportStyleChecksAsWarnings": false` in your User [[vs.1.7.docs.getstarted.settings]].
