...
As you have seen in the [[vs.1.7.api.language-extensions.programmatic-language-features]] topic, it's possible to implement Language Features by directly using `languages.*` API. Language Server Extension, however, provides an alternative way of implementing such language support.
...
![LSP Languages and Editors](/assets/lsp-languages-editors-fbq2s87engob.png)
...
- Language Client: A normal VS Code extension written in JavaScript / TypeScript. This extension has access to all [[vs.1.7.api.references.vscode-api]].
...
![LSP Illustration](/assets/lsp-illustration-7p98oj7pk2z0.png)

This guide will teach you how to build a Language Client / Server using our [Node SDK](https://github.com/microsoft/vscode-languageserver-node). The remaining document assumes that you are familiar with VS Code [[vs.1.7.api]].
...
- **[lsp-multi-server-sample](https://github.com/microsoft/vscode-extension-samples/tree/main/lsp-multi-server-sample)**: A heavily documented, advanced version of **lsp-sample** that starts a different server instance per workspace folder to support the [[vs.1.7.docs.editor.multi-root-workspaces]] feature in VS Code.
...
First look the [[vs.1.7.api.references.activation-events]]:
...
Next look at the [[vs.1.7.api.references.contribution-points#contributes.configuration]] section:
...
![Validating a text file](/assets/validation-nsb4ronu93q2.png)
...
![Debugging the client](/assets/debugging-client-k4ii3zeg2821.png)
...
![Debugging the server](/assets/debugging-server-jo7x5g7oi1yc.png)
...
![LSP Log](/assets/lsp-log-uffq1l3v9rvp.png)
...
![Maximum One Problem](/assets/validationoneproblem-nq5turub4rys.png)
...
![Code Complete](/assets/codecomplete-pfsc8q4z2zja.png)
...
- End-to-End Test: This is similar to [[vs.1.7.api.working-with-extensions.testing-extension]]. The benefit of this approach is that it runs the test by instantiating a VS Code instance with a workspace, opening the file, activating the Language Client / Server, and running [[vs.1.7.api.references.commands]]. This approach is superior if you have files, settings, or dependencies (such as `node_modules`) which are hard or impossible to mock. The popular [Python](https://github.com/microsoft/vscode-python) extension takes this approach to testing.
...
After the preparation, we can run the [[vs.1.7.api.references.commands]] corresponding to each language feature, and assert against the returned result.
...
The [[vs.1.7.api.language-extensions.programmatic-language-features]] topic describes each of the language features above and provides guidance on how to implement them either through the language server protocol or by using the extensibility API directly from your extension.
