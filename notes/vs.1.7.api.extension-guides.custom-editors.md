...
MetaDescription: >-
  Use the Custom Editor API to create customizable editors within Visual Studio
  Code.
---
...
- [[vs.1.7.api.references.vscode-api#window.registerCustomEditorProvider]]
- [[vs.1.7.api.references.vscode-api#CustomTextEditorProvider]]
...
The view side of a custom editor is implemented using a [[vs.1.7.api.extension-guides.webview]]. This lets you build the user interface of your custom editor using standard HTML, CSS, and JavaScript. Webviews cannot access the VS Code API directly but they can talk with extensions by passing messages back and forth. Check out our [[vs.1.7.api.extension-guides.webview]] for more information on webviews and best practices for working with them.

The other part of a custom editor is the document model. This model is how your extension understands the resource (file) it is working with. A `CustomTextEditorProvider` uses VS Code's standard [[vs.1.7.api.references.vscode-api#TextDocument]] as its document model and all changes to the file are expressed using VS Code's standard text editing APIs. `CustomReadonlyEditorProvider` and `CustomEditorProvider` on the other hand let you provide your own document model, which lets them be used for non-text file formats.
...
The `customEditors` [[vs.1.7.api.references.contribution-points]] is how your extension tells VS Code about the custom editors that it provides. For example, VS Code needs to know what types of files your custom editor works with as well as how to identify your custom editor in any UI.
...
Custom text editors let you create custom editors for text files. This can be anything from plain unstructured text to [CSV](https://en.wikipedia.org/wiki/Comma-separated_values) to JSON or XML. Custom text editors use VS Code's standard [[vs.1.7.api.references.vscode-api#TextDocument]] as their document model.
...
1. User clicks the **Add scratch** button in the webview. This [[vs.1.7.api.extension-guides.webview#scripts-and-message-passing]] from the webview back to the extension.
...
When the user opens a file that matches the `customEditor` contribution point, VS Code fires an `onCustomEditor` [[vs.1.7.api.references.activation-events]] and then invokes the provider registered for the provided view type. A `CustomEditorProvider` has two roles: providing the document for the custom editor and then providing the editor itself. Here's a ordered list of what happens for the `catCustoms.pawDraw` editor from the [custom editor extension sample][sample]:
