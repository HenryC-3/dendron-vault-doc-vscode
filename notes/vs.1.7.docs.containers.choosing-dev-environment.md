---
id: a8pc3hacejra3jeqnv3iar2
title: Choosing Dev Environment
desc: ''
...
MetaDescription: >-
  Guidance on choosing remote or local environments for developing and debugging
  containerized apps, using Visual Studio Code.
---
...
[[vs.1.7.docs.remote.remote-overview]] is different from your developer workstation. It can be a remote machine accessible via SSH, a virtual machine running on your developer workstation, or a development container. A remote environment can have advantages over the local environment, the main one being the ability to use the same operating system during development, and when your service is running in production. To use a remote environment, you need to ensure that `docker` command (Docker CLI) [[v[[vs.1.7.#enabling-docker-cli-inside-a-remote-development-environment]]
...
![Enable Docker inside WSL 2 distribution](/assets/devenv-enable-docker-wsl2-em4qbhoj3a4m.png)
...
1. In the Extensions view, install the Docker extension (on the remote host) (a reload may be required after this step):
   ...
   ![Screenshot - Installing the Docker extension](/assets/install-in-ssh-bbmq0ua2gtu4.png)
...
For more information about how to set up debugging inside a container see [[vs.1.7.docs.containers.quickstart-aspnet-core]], [[v[[vs.1.7.docs.containers.quickstart-node]]nd [[vs.1[[vs.1.7.docs.containers.reference]]ker-build` and `docker-run` tasks).
