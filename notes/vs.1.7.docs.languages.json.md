...
![JSON within VS Code](/assets/json_hero-rtrutz337vy3.png)
...
We also perform structural and value verification based on an associated JSON schema giving you red squiggles. To disable validation, use the `json.validate.enable` [[vs.1.7.docs.getstarted.settings]].

![IntelliSense](/assets/intellisense-02q187tmio23.png)
...
The association of a JSON file to a schema can be done either in the JSON file itself using the `$schema` attribute, or in the User or Workspace [[vs.1.7.docs.getstarted.settings]] (**File** > **Preferences** > **Settings**) under the property `json.schemas`.
...
In the following example, the JSON file specifies that its contents follow the [[vs.1.7.http:..www.coffeelint.org.]] schema.
...
The following excerpt from User [[vs.1.7.docs.getstarted.settings]] shows how `.babelrc` files are mapped to the [babelrc](https://babeljs.io/docs/usage/babelrc) schema located on [https://json.schemastore.org/babelrc](https://json.schemastore.org/babelrc).
...
Schemas and schema associations can also be defined by an extension. Check out the [[vs.1.7.api.references.contribution-points.md#contributes.jsonValidation]].
...
![Default snippets in JSON schema](/assets/defaultsnippets-q32zrxisf5aq.png)
...
If you want your descriptions to support formatting like links, you can opt in by using [[vs.1.7.docs.languages.markdown]] in your formatting with the `markdownDescription` property.
