...
MetaDescription: >-
  Python Flask tutorial showing IntelliSense, debugging, and code navigation
  support in Visual Studio Code, the best Python IDE.
...
To successfully complete this Flask tutorial, you must do the following (which are the same steps as in the [[vs.1.7.docs.python.python-tutorial]]):
...
1. In VS Code, open the Command Palette (**View** > **Command Palette** or (`kb(workbench.action.showCommands)`)). Then select the **Python: Select Interpreter** command:
    ...
    ![Flask tutorial: opening the Command Palette in VS Code](/assets/command-palette-o45gd41e7y6k.png)
...
1. The command presents a list of available interpreters that VS Code can locate automatically (your list will vary; if you don't see the desired interpreter, see [[vs.1.7.docs.python.environments]]). From the list, select the virtual environment in your project folder that starts with `./.venv` or `.\.venv`:
    ...
    ![Flask tutorial: selecting the virtual environment for Python](/assets/select-virtual-environment-zrgf95q01ict.png)
...
1. The selected environment appears on the right side of the VS Code status bar, and notice the **('.venv': venv)** indicator that tells you that you're using a virtual environment:
    ...
    ![Flask tutorial: selected environment showing in the VS Code status bar](/assets/environment-in-status-bar-3fan9gyw92h5.png)
...
1. In VS Code, create a new file in your project folder named `app.py` using either **File** > **New** from the menu, pressing `kbstyle(Ctrl+N)`, or using the new file icon in the Explorer View (shown below).
    ...
    ![Flask tutorial: new file icon in Explorer View](/assets/new-file-icon-4n94kgji7lqn.png)

1. In `app.py`, add code to import Flask and create an instance of the Flask object. If you type the code below (instead of using copy-paste), you can observe VS Code's [[vs.1.7.docs.python.editing.md#autocomplete-and-intellisense]]:
...
1. To open your default browser to the rendered page, `kbstyle(Ctrl+click)` the `http://127.0.0.1:5000/` URL in the terminal.
    ...
    ![Flask tutorial: the running app in a browser](/assets/app-in-browser-01-eowh2nprs6gp.png)
...
1. Set a breakpoint at the first line of code in the `hello_there` function (`now = datetime.now()`) by doing any one of the following:
    ...
    ![Flask tutorial: a breakpoint set on the first line of the hello_there function](/assets/debug-breakpoint-set-zcmg89j9317t.png)
...
1. Switch to **Run** view in VS Code (using the left-side activity bar or `kb(workbench.action.debug.start)`). You may see the message "To customize Run and Debug create a launch.json file". This means that you don't yet have a `launch.json` file containing debug configurations. VS Code can create that for you if you click on the **create a launch.json file** link:
    ...
    ![Flask tutorial: initial view of the debug panel](/assets/debug-panel-initial-view-6jl5u5ojirf5.png)
...
1. Save `launch.json` (`kb(workbench.action.files.save)`). In the debug configuration dropdown list select the **Python: Flask** configuration.
    ...
    ![Flask tutorial: selecting the Flask debugging configuration](/assets/debug-select-configuration-444sabvp9p3w.png)
...
1. Start the debugger by selecting the **Run** > **Start Debugging** menu command, or selecting the green **Start Debugging** arrow next to the list (`kb(workbench.action.debug.continue)`):
    ...
    ![Flask tutorial: start debugging/continue arrow on the debug toolbar](/assets/debug-continue-arrow-f3ue7ddu52f4.png)

    Observe that the status bar changes color to indicate debugging:

    ![Flask tutorial: appearance of the debugging status bar](/assets/debug-status-bar-odxcgdsb2rsz.png)
    ...
    ![Flask tutorial: the VS Code debug toolbar](/assets/debug-toolbar-iwlaqxnrur9l.png)
...
1. Output appears in a "Python Debug Console" terminal. `kbstyle(Ctrl+click)` the `http://127.0.0.1:5000/` link in that terminal to open a browser to that URL. In the browser's address bar, navigate to `http://127.0.0.1:5000/hello/VSCode`. Before the page renders, VS Code pauses the program at the breakpoint you set. The small yellow arrow on the breakpoint indicates that it's the next line of code to run.
    ...
    ![Flask tutorial: VS Code paused at a breakpoint](/assets/debug-program-paused-7dmz00nlnab4.png)
...
1. On the left side of the VS Code window, you see a **Variables** pane that shows local variables, such as `now`, as well as arguments, such as `name`. Below that are panes for **Watch**, **Call Stack**, and **Breakpoints** (see [[vs.1.7.docs.editor.debugging]] for details). In the **Locals** section, try expanding different values. You can also double-click values (or use `kb(debug.setVariable)`) to modify them. Changing variables such as `now`, however, can break the program. Developers typically make changes only to correct values when the code didn't produce the right value to begin with.
    ...
    ![Flask tutorial: local variables and arguments in VS Code during debugging](/assets/debug-local-variables-xytmmhmh71cc.png)
...
1. Step through a few more lines of code, if you'd like, then select Continue (`kb(workbench.action.debug.continue)`) to let the program run. The browser window shows the result:
    ...
    ![Flask tutorial: result of the modified program](/assets/debug-run-result-j46x9uj7j38n.png)
...
- **Peek Definition** (`kb(editor.action.peekDefinition)`, also on the right-click context menu), is similar, but displays the class definition directly in the editor (making space in the editor window to avoid obscuring any code). Press `kbstyle(Escape)` to close the Peek window or use the **x** in the upper right corner.
    ...
    ![Flask tutorial: peek definition showing the Flask class inline](/assets/peek-definition-k6fmc2msvqh6.png)
...
1. In the `templates` folder, create a new file named `home.html`, Then start typing `flext` to see the snippet appear as a completion:
    ...
    ![Flask tutorial: autocompletion for the flextlayout code snippet](/assets/autocomplete-for-code-snippet-gz3pvigbg07c.png)

    When you select the completion, the snippet's code appears with the cursor on the snippet's insertion point:

    ![Flask tutorial: insertion of the flextlayout code snippet](/assets/code-snippet-inserted-9wxjj6gabgzu.png)
...
![Flask tutorial: app rendering a common nav bar from the base template](/assets/full-app-o61ja0vt2imh.png)
...
1. Your project's structure should now be similar to the following:
    ...
    ![Flask tutorial: modified project structure with separate files and folders for parts of the app](/assets/project-structure-fbv7t0srhknj.png)
