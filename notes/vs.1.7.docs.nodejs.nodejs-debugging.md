...
MetaDescription: >-
  The Visual Studio Code editor includes Node.js debugging support. Set
...
![Auto Attach](/assets/auto-attach-ihx249deg588.gif)
...
In `smart` Auto Attach mode, VS Code will try to attach to your code, and not attach to build tools you aren't interested in debugging. It does this by matching the main script against a list of [[vs.1.7.docs.editor.codebasics.md#advanced-search-options]]. The glob patterns are configurable in the **debug.javascript.autoAttachSmartPattern** setting, which defaults to:
...
In this section we'll go into more detail about configurations and features for more advanced debugging scenarios. You'll find instruction for debugging with [[vs.1.7.docs.nodejs.nodejs-debugging.md#source-maps]], [[vs.1.7.docs.nodejs.nodejs-debugging.md#skipping-uninteresting-code]], doing [[vs.1.7.docs.nodejs.nodejs-debugging.md#remote-debugging]], and much more.
...
Debugging configurations are stored in a `launch.json` file located in your workspace's `.vscode` folder. An introduction into the creation and use of debugging configuration files is in the general [[vs.1.7.docs.editor.debugging.md#launch-configurations]] article.
...
* `outFiles` - array of glob patterns for locating generated JavaScript files. See section [[vs.1.7.docs.nodejs.nodejs-debugging.md#source-maps]].
* `resolveSourceMapLocations` - an array of glob patterns for locations where source maps should be parsed. See section [[vs.1.7.docs.nodejs.nodejs-debugging.md#source-maps]].
* `timeout` - when restarting a session, give up after this number of milliseconds. See section [[vs.1.7.docs.nodejs.nodejs-debugging.md#attaching-to-nodejs]].
* `stopOnEntry` - break immediately when the program launches.
* `localRoot` - VS Code's root directory. See section [[vs.1.7.docs.nodejs.nodejs-debugging.md#remote-debugging]] below.
* `remoteRoot` - Node's root directory. See section [[vs.1.7.docs.nodejs.nodejs-debugging.md#remote-debugging]] below.
* `smartStep`- try to automatically step over code that doesn't map to source files. See section [[vs.1.7.docs.nodejs.nodejs-debugging.md#smart-stepping]].
* `skipFiles` - automatically skip files covered by these glob patterns. See section [[vs.1.7.docs.nodejs.nodejs-debugging.md#skipping-uninteresting-code]].
...
* `runtimeExecutable` - absolute path to the runtime executable to be used. Default is `node`. See section [[vs.1.7.docs.nodejs.nodejs-debugging.md#launch-configuration-support-for-npm-and-other-tools]].
* `runtimeArgs` - optional arguments passed to the runtime executable.
* `runtimeVersion` - if "[nvm](https://github.com/creationix/nvm)" (or "[nvm-windows](https://github.com/coreybutler/nvm-windows)") or "[nvs](https://github.com/jasongin/nvs)" is used for managing Node.js versions, this attribute can be used to select a specific version of Node.js. See section [[vs.1.7.docs.nodejs.nodejs-debugging.md#multi-version-support]] below.
...
* `envFile` - optional path to a file containing environment variable definitions. See section [[vs.1.7.docs.nodejs.nodejs-debugging.md#load-environment-variables-from-external-file]] below.
* `console` - the console to launch the program (`internalConsole`, `integratedTerminal`, `externalTerminal`). See section [[vs.1.7.docs.nodejs.nodejs-debugging.md#node-console]] below.
...
* `restart` - restart the connection on termination. See section [[vs.1.7.docs.nodejs.nodejs-debugging.md#restarting-debug-sessions-automatically-when-source-is-edited]].
* `port` - debug port to use. See sections [[vs.1.7.docs.nodejs.nodejs-debugging.md#attaching-to-nodejs]] and [[vs.1.7.docs.nodejs.nodejs-debugging.md#remote-debugging]].
* `address` - TCP/IP address of the debug port. See sections [[vs.1.7.docs.nodejs.nodejs-debugging.md#attaching-to-nodejs]] and [[vs.1.7.docs.nodejs.nodejs-debugging.md#remote-debugging]].
...
![Launch configuration snippets for Node.js](/assets/launch-snippets-0w5lwodnjzgw.png)

You can also bring up the snippets with the **Add Configuration...** button in the lower right of the `launch.json` editor window.

![Add Configuration button](/assets/add-configuration-button-l4p2g33ncvoi.png)
...
![Node.js Process picker](/assets/process-picker-iup32z2y0okp.png)
...
> **Note:** VS Code now has universal [[vs.1.7.docs.remote.remote-overview]]. Using the [Remote Development](https://aka.ms/vscode-remote/download/extension) extensions, Node.js development in remote scenarios and containers is no different than Node.js development in a local setup. **This is the recommended way to remote debug Node.js programs**. Check out the [[v[[vs.1.7.docs.remote.remote-overview.md#getting-started]]ction and [[v[[vs.1.7.docs.remote.remote-overview.md#remote-tutorials]] learn more.
...
![Loaded Scripts Explorer](/assets/loaded-scripts-explorer-57o66rh335ie.gif)
...
![Conditional breakpoint](/assets/conditional-breakpoint-8njkl6xd2ta9.gif)
...
![Logpoint](/assets/logpoint-breakpoint-99jc9b59qe3u.gif)
...
![Hit count breakpoint](/assets/hit-count-breakpoint-gpdvb8aiu9zt.gif)
...
![Breakpoints View](/assets/breakpointsvalidation-h50b514twude.png)
...
![Breakpoint Actions](/assets/breakpointstoolbar-wp9vktkf9jlu.png)
...
![Skipped source is dimmed in call stack view](/assets/dimmed-callstack-dfagewzqfshi.png)
...
* [[vs.1.7.docs.nodejs.nodejs-tutorial]] - End to end Node scenario with a sample application

To see tutorials on the basics of Node.js debugging, check out these videos:

* [[vs.1.7.docs.introvideos.debugging]] - Introductory video showcasing the basics of debugging.
...
* [[vs.1.7.api.extension-guides.debugger-extension]] - Steps to create a VS Code debug extension starting from a mock sample
