---
id: 7hxviompupjqu1bpp6kwsy3
title: Tree View
desc: ''
updated: 1660824078429
created: 1660824078429
isDir: false
...
The Tree View API allows extensions to show content in the sidebar in Visual Studio Code. This content is structured as a tree and conforms to the style of the [[vs.1.7.docs.getstarted.userinterface#_views]] of VS Code.

For example, the built-in References Search View extension shows reference search results as a separate view.

![References Search View](/assets/references-search-tree-view-m6r4d8py4y1c.png)
...
First you have to let VS Code know that you are contributing a view, using the [[vs.1.7.api.references.contribution-points#contributes.views]] Contribution Point in `package.json`.
...
- `getTreeItem(element: T): TreeItem | Thenable<TreeItem>` - Implement this to return the UI representation ([[vs.1.7.api.references.vscode-api#TreeItem]]) of the element that gets displayed in the view.
...
- `vscode.window.createTreeView` - Create the Tree View by providing the registered view ID and above data provider. This will give access to the [[vs.1.7.api.references.vscode-api#TreeView]], which you can use for performing other view operations. Use `createTreeView`, if you need the `TreeView` API.
...
![View](/assets/view-96440xuxjn54.png)
...
It is important that your extension is activated only when user needs the functionality that your extension provides. In this case, you should consider activating your extension when the user starts using the view. VS Code emits an activationEvent [[vs.1.7.api.references.activation-events#onView]] (`onView:nodeDependencies` for the example above) when the user opens the view.
...
![View Container](/assets/view-container-3ulr241ntd7z.png)

To contribute a View Container, you should first register it using [[vs.1.7.api.references.contribution-points#contributes.viewsContainers]] Contribution Point in `package.json`.
...
Once you've created a View Container, you can use the [[vs.1.7.api.references.contribution-points#contributes.views]] Contribution Point in `package.json`.
...
![Views Menu](/assets/views-menu-askkqk9wdf5x.png)
...
You can control the visibility of these actions using a [[vs.1.7.api.references.when-clause-contexts]].

![View Actions](/assets/view-actions-av214ssrbgif.png)
  ...
  "viewsWelcome": [
    {
      ...
      "contents": "No node dependencies found [learn more](https://www.npmjs.com/).\n[[vs.1.7.command:nodeDependencies.addEntry]]"
    }
  ]
}
```

![Welcome Content](/assets/welcome-content-h2w6ezhjtrhu.png)
...
Extension writers should register a [[vs.1.7.api.references.vscode-api#TreeDataProvider]] programmatically to populate data in the view.
