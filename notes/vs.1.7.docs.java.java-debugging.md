...
>For details on how to get started with the extension pack, you can review the [[vs.1.7.docs.java.java-tutorial]] tutorial.
...
![Debug Menu](/assets/run-debug-button-1jqlvo9q3nuc.png)

The `launch.json` file is located in a `.vscode` folder in your workspace (project root folder).

For more details on how to create the `launch.json`, read [[vs.1.7.docs.editor.debugging.md#launch-configurations]]; for more details on configuration options for Java, you can read [[vs.1.7.docs.java.java-debugging.md#configuration-options]].
...
You will find **Run|Debug** on the [[vs.1.7.blogs.2017.02.12.code-lens-roundup]] of your `main()` function.

![CodeLens](/assets/java-codelens-6cvkwj0h3hq9.png)
...
![EditorMenu](/assets/run-menu-uz945ei0omt6.png)

### Run from pressing F5

Pressing `kb(workbench.action.debug.start)`, the debugger will automatically find the entry point of your project and start debugging. You can also start a debugging session from the **Run and Debug** view from the side bar of VS Code. See more at [[vs.1.7.docs.editor.debugging]].
...
![Data Breakpoint](/assets/data-breakpoint-18g3gpt4s3yh.png)

### Breakpoint - Logpoints

[[vs.1.7.blogs.2018.07.12.introducing-logpoints-and-auto-attach.md#introducing-logpoints]] is also supported by Java Debugger. Logpoints allow you to send output to Debug Console without editing code. They're different from breakpoints because they don't stop the execution flow of your application.
...
Below are all the configurations available for `Launch` and `Attach`. For more information about how to write the `launch.json` file, refer to [[vs.1.7.docs.editor.debugging]].
...
- [[vs.1.7.docs.editor.debugging]] - Find out how to use the debugger in VS Code with your project for any language.

And for Java:

- [[vs.1.7.docs.java.java-testing]] - Test Java within VS Code with the Java Test Runner extension.
- [[vs.1.7.docs.java.extensions]] - Learn about more useful Java extensions for VS Code.
